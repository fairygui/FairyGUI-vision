...
void Window::handleInit()
{
    ...
    addListener(UIEventType::TouchBegin, CALLBACK_1(Window::onTouchBegin, this));
    addListener(UIEventType::AddedToStage, CALLBACK_1(Window::onAddedToStage, this));
    addListener(UIEventType::RemoveFromStage, CALLBACK_1(Window::onRemoveFromStage, this));
}
...
void Window::setCloseButton(GObject * value)
{
    ...
    {
        _closeButton->retain();
        _closeButton->addClickListener(CALLBACK_1(Window::closeEventHandler, this), EventTag(this));
    }
}
...
void Window::setDragArea(GObject * value)
{
    if (_dragArea != value)
    {
        if (_dragArea != nullptr)
        {
            ...
            _dragArea->removeListener(UIEventType::DragStart, EventTag(this));
            _dragArea->release();
        }
        ...
        if (_dragArea != nullptr)
        {
            ...
            if (dynamic_cast<GGraph*>(_dragArea) && ((GGraph*)_dragArea)->isEmpty())
                ((GGraph*)_dragArea)->drawRect(_dragArea->getWidth(), _dragArea->getHeight(), 0, VColorRef(0, 0, 0, 0), VColorRef(0, 0, 0, 0));
            _dragArea->setDraggable(true);
            _dragArea->addListener(UIEventType::DragStart, CALLBACK_1(Window::onDragStart, this), EventTag(this));
        }
    }
}
...
void Window::layoutModalWaitPane()
{
    ...
    {
        hkvVec2 pt = _frame->localToGlobal(hkvVec2::ZeroVector());
        pt = globalToLocal(pt);
        ...
        _modalWaitPane->setSize(_size.x, _size.y);
}
...
void Window::onAddedToStage(EventContext*)
{
    if (!_inited)
        initWindow();
    else
...
void Window::onRemoveFromStage(EventContext*)
{
    closeModalWait();
    onHide();
}
