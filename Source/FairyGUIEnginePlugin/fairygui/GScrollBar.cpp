...
void GScrollBar::setDisplayPerc(float value)
{
    if (_vertical)
    {
        ...
        _grip->setY(hkvMath::round(_bar->getY() + (_bar->getHeight() - _grip->getHeight()) * _scrollPerc));
    }
    else
    {
        if (!_fixedGripSize)
            _grip->setWidth(floor(value * _bar->getWidth()));
        _grip->setX(hkvMath::round(_bar->getX() + (_bar->getWidth() - _grip->getWidth()) * _scrollPerc));
    }
}
...
void GScrollBar::constructFromXML(TXMLElement * xml)
{
    ...
    _grip->addListener(UIEventType::TouchBegin, CALLBACK_1(GScrollBar::onGripTouchBegin, this));
    _grip->addListener(UIEventType::TouchMove, CALLBACK_1(GScrollBar::onGripTouchMove, this));
    addListener(UIEventType::TouchBegin, CALLBACK_1(GScrollBar::onTouchBegin, this));

    if (_arrowButton1 != nullptr)
        _arrowButton1->addListener(UIEventType::TouchBegin, CALLBACK_1(GScrollBar::onArrowButton1Click, this));
    if (_arrowButton2 != nullptr)
        _arrowButton2->addListener(UIEventType::TouchBegin, CALLBACK_1(GScrollBar::onArrowButton2Click, this));
}
...
void GScrollBar::onTouchBegin(EventContext * context)
{
    ...
    hkvVec2 pt = _grip->globalToLocal(evt->getPosition());
    if (_vertical)
    {
        if (pt.y < 0)
...
{
    hkvVec2 pt = globalToLocal(context->getInput()->getPosition());
