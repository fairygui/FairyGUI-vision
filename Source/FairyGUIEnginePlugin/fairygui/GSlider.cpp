...
void GSlider::update()
{
    float percent = MIN((float)(_value / _max), 1.0f);
...
void GSlider::updateWidthPercent(float percent)
{
    ...
    if (!_reverse)
    {
        ...
        {
            _barObjectH->setWidth(hkvMath::round(fullWidth * percent));
        }
        if (_barObjectV != nullptr)
        {
            _barObjectV->setHeight(hkvMath::round(fullHeight * percent));
        }
    }
    else
    {
...
void GSlider::constructFromXML(TXMLElement * xml)
{
    ...
    {
        _gripObject->addListener(UIEventType::TouchBegin, CALLBACK_1(GSlider::onGripTouchBegin, this));
        _gripObject->addListener(UIEventType::TouchMove, CALLBACK_1(GSlider::onGripTouchMove, this));
    }

    addListener(UIEventType::TouchBegin, CALLBACK_1(GSlider::onTouchBegin, this));
}
...
void GSlider::onTouchBegin(EventContext * context)
{
    ...
    hkvVec2 pt = _gripObject->globalToLocal(evt->getPosition());
