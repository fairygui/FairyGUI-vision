...
std::unordered_map<std::string, UIObjectFactory::GComponentCreator> UIObjectFactory::_packageItemExtensions;
UIObjectFactory::GLoaderCreator UIObjectFactory::_loaderCreator;

void UIObjectFactory::setPackageItemExtension(const std::string& url, GComponentCreator creator)
{
    if (url.size() == 0)
    {
        CCLOGERROR("Invaild url: %s", url.c_str());
        return;
    }
...
GObject * UIObjectFactory::newObject(const std::string & type)
{
    if (type == "image")
