...
bool TreeView::init(GList * list)
{
    _list = list;
    _list->retain();
    _list->addListener(UIEventType::ClickItem, CALLBACK_1(TreeView::onClickItem, this));
    _list->addListener(UIEventType::RightClickItem, CALLBACK_1(TreeView::onClickItem, this));
...
void TreeView::createCell(TreeNode * node)
{
    ...
    if (expandButton != nullptr)
    {
        if (node->isFolder())
        {
            ...
            expandButton->addClickListener(CALLBACK_1(TreeView::onClickExpandButton, this));
