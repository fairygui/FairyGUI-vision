...
class FGUI_IMPEXP GList : public GComponent
{
public:
    ...
    GObject* getFromPool() { return getFromPool(""); }
    GObject* getFromPool(const std::string& url);
    void returnToPool(GObject *obj);
    GObject* addItemFromPool() { return addItemFromPool(""); }
    ...
    virtual hkvVec2 getSnappingPosition(const hkvVec2& pt) override;
...
private:
    ...
    void doRefreshVirtualList(float);
...
private:
    CC_DISALLOW_COPY_AND_ASSIGN(GList);
};
