...
bool PopupMenu::init(const std::string & resourceURL)
{
    ...
    _contentPane->retain();
    _contentPane->addListener(UIEventType::AddedToStage, CALLBACK_1(PopupMenu::onAddedToStage, this));
    ...
    _list->addListener(UIEventType::ClickItem, CALLBACK_1(PopupMenu::onClickItem, this));
...
GButton * PopupMenu::addItem(const std::string & caption, EventCallback callback)
{
    ...
    item->removeListener(UIEventType::ClickMenu);
    if (callback)
        item->addListener(UIEventType::ClickMenu, callback);
...
void PopupMenu::clearItems()
{
    ...
    for (int i = 0; i < cnt; i++)
        _list->getChildAt(i)->removeListener(UIEventType::ClickMenu);
    _list->removeChildrenToPool();
}
...
void PopupMenu::onAddedToStage(EventContext * context)
{
    _list->setSelectedIndex(-1);
