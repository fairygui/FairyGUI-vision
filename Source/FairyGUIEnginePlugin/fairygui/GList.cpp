...
GList::ItemInfo::ItemInfo() :
    obj(nullptr), updateFlag(0), selected(false), size(0, 0)
{
}
...
GObject * GList::addChildAt(GObject * child, int index)
{
    ...
    child->addClickListener(CALLBACK_1(GList::onClickItem, this), EventTag(this));
    child->addListener(UIEventType::TouchBegin, CALLBACK_1(GList::onItemTouchBegin, this), EventTag(this));
    child->addListener(UIEventType::RightClick, CALLBACK_1(GList::onClickItem, this), EventTag(this));
...
void GList::removeChildAt(int index)
{
    ...
    child->removeListener(UIEventType::TouchBegin, EventTag(this));
    child->removeListener(UIEventType::RightClick, EventTag(this));
    ...
    else
    {
        if (evt->isShiftDown())
        {
            if (!button->isSelected())
            {
                ...
                {
                    int min = hkvMath::Min<int>(_lastSelectedIndex, index);
                    int max = hkvMath::Max<int>(_lastSelectedIndex, index);
                    max = hkvMath::Min<int>(max, getNumItems() - 1);
                    if (_virtual)
                    {
...
void GList::resizeToFit(int itemCount, int minSize)
{
    ...
    if (_virtual)
    {
        int lineCount = (int)ceil((float)itemCount / _curLineItemCount);
        if (_layout == ListLayoutType::SINGLE_COLUMN || _layout == ListLayoutType::FLOW_HORIZONTAL)
            setViewHeight(lineCount * _itemSize.y + hkvMath::Max<int>(0, lineCount - 1) * _lineGap);
        else
            setViewWidth(lineCount * _itemSize.x + hkvMath::Max<int>(0, lineCount - 1) * _columnGap);
    }
    else if (itemCount == 0)
    {
        if (_layout == ListLayoutType::SINGLE_COLUMN || _layout == ListLayoutType::FLOW_HORIZONTAL)
...
{
    if (_selectionController != nullptr && !_selectionController->isChanging()
...
void GList::scrollToView(int index, bool ani, bool setFirst)
{
    if (_virtual)
    {
        ...
        if (_loop)
            index = (int)floor(_firstIndex / _numItems) * _numItems + index;
        ...
        if (_layout == ListLayoutType::SINGLE_COLUMN || _layout == ListLayoutType::FLOW_HORIZONTAL)
        {
            ...
            rect.Set(0, pos, _itemSize.x, pos + ii.size.y);
        }
        else if (_layout == ListLayoutType::SINGLE_ROW || _layout == ListLayoutType::FLOW_VERTICAL)
        {
            ...
            rect.Set(pos, 0, pos + ii.size.x, _itemSize.y);
        }
        else
        {
            int page = index / (_curLineItemCount * _curLineItemCount2);
            rect.m_vMin.x = page * getViewWidth() + (index % _curLineItemCount) * (ii.size.x + _columnGap);
            rect.m_vMin.y = (index / _curLineItemCount) % _curLineItemCount2 * (ii.size.y + _lineGap);
            rect.m_vMax.x = rect.m_vMin.x + ii.size.x;
            rect.m_vMax.y = rect.m_vMin.y + ii.size.y;
        }
...
void GList::setVirtual(bool loop)
{
    if (!_virtual)
    {
        ...
        addListener(UIEventType::Scroll, CALLBACK_1(GList::onScroll, this));
        setVirtualListChangedFlag(true);
    }
}
...
void GList::setNumItems(int value)
{
    if (_virtual)
    {
        ...
        if (_virtualListChanged != 0)
            scheduleOnce(SCHEDULE_SELECTOR(GList::doRefreshVirtualList));

        //Á¢¼´Ë¢ÐÂ
        doRefreshVirtualList(0);
    }
    else
    {
        int cnt = (int)_children.size();
...
hkvVec2 GList::getSnappingPosition(const hkvVec2 & pt)
{
    if (_virtual)
    {
        hkvVec2 ret = pt;
...
void GList::checkVirtualList()
{
    ...
    {
        doRefreshVirtualList(0);
        unSchedule(SCHEDULE_SELECTOR(GList::doRefreshVirtualList));
    }
}
...
void GList::doRefreshVirtualList(float)
{
    ...
    if (layoutChanged)
    {
        ...
        else if (_layout == ListLayoutType::FLOW_HORIZONTAL)
        {
            ...
            {
                _curLineItemCount = (int)floor((_scrollPane->getViewSize().x + _columnGap) / (_itemSize.x + _columnGap));
        ...
        else if (_layout == ListLayoutType::FLOW_VERTICAL)
        {
            ...
            {
                _curLineItemCount = (int)floor((_scrollPane->getViewSize().y + _lineGap) / (_itemSize.y + _lineGap));
                if (_curLineItemCount <= 0)
                    _curLineItemCount = 1;
            }
        }
        else //pagination
        {
            ...
            {
                _curLineItemCount2 = (int)floor((_scrollPane->getViewSize().y + _lineGap) / (_itemSize.y + _lineGap));
    ...
    if (_realNumItems > 0)
    {
        int len = (int)ceil((float)_realNumItems / _curLineItemCount) * _curLineItemCount;
        int len2 = hkvMath::Min<int>(_curLineItemCount, _realNumItems);
        if (_layout == ListLayoutType::SINGLE_COLUMN || _layout == ListLayoutType::FLOW_HORIZONTAL)
        {
        ...
        {
            int pageCount = (int)ceil((float)len / (_curLineItemCount * _curLineItemCount2));
...
int GList::getIndexOnPos3(float & pos, bool forceUpdate)
{
    ...
    int page = (int)floor(pos / viewWidth);
...
void GList::handleScroll1(bool forceUpdate)
{
    ...
    float max = pos + _scrollPane->getViewSize().y;
    bool end = max == _scrollPane->getContentSize().y;
    ...
    int partSize = (int)((_scrollPane->getViewSize().x - _columnGap * (_curLineItemCount - 1)) / _curLineItemCount);
    ...
    if (curIndex > 0 && numChildren() > 0 && _container->getY() < 0 && getChildAt(0)->getY() > -_container->getY())
        handleScroll1(false);
}
...
void GList::handleScroll2(bool forceUpdate)
{
    ...
    float max = pos + _scrollPane->getViewSize().x;
    bool end = pos == _scrollPane->getContentSize().x;
    ...
    float firstItemDeltaSize = 0;
    std::string url = _defaultItem;
    int partSize = (int)((_scrollPane->getViewSize().y - _lineGap * (_curLineItemCount - 1)) / _curLineItemCount);
    ...
    if (curIndex > 0 && numChildren() > 0 && _container->getX() < 0 && getChildAt(0)->getX() > -_container->getX())
        handleScroll2(false);
}
...
void GList::handleScroll3(bool forceUpdate)
{
    ...
    bool needRender;
    std::string url = _defaultItem;
    int partWidth = (int)((_scrollPane->getViewSize().x - _columnGap * (_curLineItemCount - 1)) / _curLineItemCount);
    int partHeight = (int)((_scrollPane->getViewSize().y - _lineGap * (_curLineItemCount2 - 1)) / _curLineItemCount2);
    _itemInfoVer++;
...
void GList::handleAlign(float contentWidth, float contentHeight)
{
    ...
    if (contentHeight < viewHeight)
    {
        if (_verticalAlign == VertAlignType::MIDDLE)
            newOffset.y = hkvMath::floor((viewHeight - contentHeight) *0.5f);
    ...
    if (contentWidth < viewWidth)
    {
        if (_align == AlignType::CENTER)
            newOffset.x = hkvMath::floor((viewWidth - contentWidth) * 0.5f);
            ...
            for (i = 0; i < cnt; i++)
            {
                ...
                if (j == _columnCount || i == cnt - 1)
                {
                    ...
                    for (j = lineStart; j <= i; j++)
                    {
                        ...
                        if (j < i)
                        {
                            child->setSize(child->sourceSize.x + hkvMath::round(child->sourceSize.x * ratio), child->getHeight(), true);
            ...
            for (i = 0; i < cnt; i++)
            {
                ...
                if (j == _lineCount || i == cnt - 1)
                {
                    ...
                    for (j = lineStart; j <= i; j++)
                    {
                        ...
                        if (j < i)
                        {
                            child->setSize(child->getWidth(), child->sourceSize.y + hkvMath::round(child->sourceSize.y * ratio), true);
            ...
            for (i = 0; i < cnt; i++)
            {
                ...
                if (j == _columnCount || i == cnt - 1)
                {
                    ...
                    for (j = lineStart; j <= i; j++)
                    {
                        ...
                        if (j < i)
                        {
                            child->setSize(child->sourceSize.x + hkvMath::round(child->sourceSize.x * ratio),
