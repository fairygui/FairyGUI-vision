...
V_IMPLEMENT_ACTION("fgui", MyAction_cl, VAction, &g_myComponentModule, NULL)
...
VBool MyAction_cl::Do(const class VArgList &argList)
{
    ...
    if (argList.GetArgCount() == 0)
        return TRUE;

    if (strcmp(argList.AsString(1), "cursor") == 0)
    {
        if (FGUIManager::GlobalManager().isShowCursor())
            FGUIManager::GlobalManager().setShowCursor(false);
        else
            FGUIManager::GlobalManager().setShowCursor(true);
        Print("command ok.");
    }
    else
        Print("Unkown command %s", argList.AsString(1));
